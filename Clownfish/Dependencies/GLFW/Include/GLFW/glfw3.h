...
#define GLFW_MOUSE_BUTTON_LAST      MOUSE_BUTTON_8
 ...
 *
 *  If the key is `KEY_UNKNOWN`, the scancode is used instead, otherwise
 *  the scancode is ignored.  If a non-printable key or (if the key is
 *  `KEY_UNKNOWN`) a scancode that maps to a non-printable key is
