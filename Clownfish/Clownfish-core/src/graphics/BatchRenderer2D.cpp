		...
		void BatchRenderer2D::drawString(const std::string& text, float textfieldsize, const Font& font, const maths::vec3& position, unsigned int color)
		{
			...
			for (int i = 0; i < text.length(); i++)
			{
				...
				if (glyph != NULL)
				{
					...
					m_Buffer->vertex = m_TransformationStack.back() * maths::vec3(x0, y0, 0);
					m_Buffer->uv = maths::vec2(u0, v0);
					m_Buffer->tid = ts;
					m_Buffer->color = color;
					m_Buffer++;

					m_Buffer->vertex = m_TransformationStack.back() *maths::vec3(x0, y1, 0);
					m_Buffer->uv = maths::vec2(u0, v1);
					m_Buffer->tid = ts;
					m_Buffer->color = color;
					m_Buffer++;

					m_Buffer->vertex = m_TransformationStack.back() * maths::vec3(x1, y1, 0);
					m_Buffer->uv = maths::vec2(u1, v1);
					m_Buffer->tid = ts;
					m_Buffer->color = color;
					m_Buffer++;

					m_Buffer->vertex = m_TransformationStack.back() * maths::vec3(x1, y0, 0);
